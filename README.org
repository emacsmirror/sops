...
To create a new encrypted file: see =sops-find-file= in future work (not yet shipped).
...
v0.2 is a clean break. If your v1 config used:
...
- v0.2 has no temp buffer.  Both hooks run in the same buffer that
...
- future work: =M-x sops-find-file= — open existing or create new encrypted file
